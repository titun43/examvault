...
class FirestoreService {
  ...
  // ==================== PREMIUM PROPAGATION ====================
  // THE ROOT-CAUSE FIX for "category premium but inner tests accessible".
  //
  // Tests do NOT have a `categoryId` field — they only have `subjectId`.
  // So to mark every test inside a premium category as premium, we must
  // resolve tests THROUGH the subjects collection:
  //   subjects.where(categoryId == X) → collect subjectIds
  //   tests.where(subjectId in [subjectIds]) → batch update isPremium
  //
  // Firestore's `in` query supports max 30 values, so we chunk subjectIds
  // into batches of 30. Returns the number of tests updated.
  static Future<int> propagateCategoryPremiumToTests(
    String categoryId,
    bool isPremium,
  ) async {
    // 1. Find the category to also match by name/slug (robust matching,
    //    mirroring getSubjects — admin may have used name/slug as the
    //    subjects.categoryId value).
    String? catName;
    String? catSlug;
    try {
      final catDoc = await _db.collection('categories').doc(categoryId).get();
      if (catDoc.exists) {
        final d = catDoc.data()!;
        catName = d['name']?.toString();
        catSlug = d['slug']?.toString();
      }
    } catch (e) {
      print('propagate: fetch category error: $e');
    }

    // 2. Collect all subject ids whose categoryId matches (by id, name, slug).
    final candidates = <String>{
      categoryId,
      if (catName != null && catName.isNotEmpty) catName,
      if (catSlug != null && catSlug.isNotEmpty) catSlug,
    }.toList();
    ...
    for (final cand in candidates) {
      try {
        ...
        print('propagate: subjects query "$cand" error: $e');
      }
    }

    if (subjectIds.isEmpty) return 0;

    // 3. Query tests by subjectId in chunks of 30 (Firestore `in` limit).
    final subjectIdList = subjectIds.toList();
    int updated = 0;
    for (int i = 0; i < subjectIdList.length; i += 30) {
      final chunk = subjectIdList.sublist(
        i,
        (i + 30 > subjectIdList.length) ? subjectIdList.length : i + 30,
      );
      try {
        final snap = await _db
            .collection('tests')
        ...
        for (final doc in snap.docs) {
          batch.update(doc.reference, {'isPremium': isPremium});
        }
        await batch.commit();
        updated += snap.docs.length;
      } catch (e) {
        print('propagate: tests batch error: $e');
      }
    }
    return updated;
  }

  /// Re-runs [propagateCategoryPremiumToTests] for EVERY category. Used by the
  /// admin "Re-sync premium" button to repair legacy tests that were left
  /// `isPremium=false` by the old buggy propagation. Returns total tests
  /// updated across all categories.
  static Future<int> resyncAllCategoriesPremium() async {
    try {
      final cats = await getCategories();
      int total = 0;
      for (final cat in cats) {
        try {
          total += await propagateCategoryPremiumToTests(
            cat.id,
            cat.isPremium,
          );
        } catch (e) {
          print('resync: category ${cat.id} error: $e');
        }
      }
      return total;
    } catch (e) {
      print('resyncAllCategoriesPremium error: $e');
      return 0;
    }
  }
